...
const Registration = require(path.join(__dirname, 'models', 'Registration'));
...
const MONGO_URI = process.env.MONGO_URI || 'mongodb://Chikzz007:Luckyboy007@ac-figbyvt-shard-00-00.2szg0fu.mongodb.net:27017,ac-figbyvt-shard-00-01.2szg0fu.mongodb.net:27017,ac-figbyvt-shard-00-02.2szg0fu.mongodb.net:27017/?ssl=true&replicaSet=atlas-13wcso-shard-0&authSource=admin&appName=Cluster0';
...
app.post('/register', async (req, res) => {
  try {
    ...
    if (!allowedEvents.includes(normalizedEventName)) {
      return res.status(400).json({
        success: false,
        message: `Invalid eventName value. Allowed values: ${allowedEvents.join(', ')}`,
      });
    }

    if (!allowedSlots.includes(normalizedTimeSlot)) {
      return res.status(400).json({
        success: false,
        message: `Invalid timeSlot value. Allowed values: ${allowedSlots.join(', ')}`,
      });
    }
